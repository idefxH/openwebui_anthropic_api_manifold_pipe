...
>
> 💡 **Cache Logic**: All cache breakpoints are applied via a single centralized method (`_apply_cache_control`) right before each API call. During tool loops, the cache breakpoint moves forward to the latest content. Programmatic tool calling uses assistant blocks instead of tool_results for cache placement.
